...
document.getElementById("loginForm").onsubmit = async event => { event.preventDefault(); loginMessage.textContent = ""; const form = new FormData(event.currentTarget); try { await signInWithEmailAndPassword(auth, form.get("email"), form.get("password")); } catch (error) { loginMessage.textContent = "Đăng nhập thất bại. Kiểm tra email, mật khẩu và Authentication trong Firebase."; } };
